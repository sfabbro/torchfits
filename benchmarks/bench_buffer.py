...
class BufferBenchmark:
    ...
    def run_memory_pool_benchmarks(self) -> Dict[str, Any]:
        ...
        for pool_size in self.pool_sizes:
            buffer.configure_buffers(
                buffer_size_mb=pool_size, num_buffers=8, enable_memory_pool=True
            )
    ...
    def run_streaming_buffer_benchmarks(self) -> Dict[str, Any]:
        ...
        for capacity in self.streaming_capacities:
            ...
            print(
                f"    Capacity {capacity}: {len(test_data) / streaming_time:.1f} ops/sec"
            )
    ...
    def run_workload_optimization_benchmarks(self) -> Dict[str, Any]:
        ...
        for workload in workloads:
            ...
            buffer.optimize_for_workload(
                workload["file_size_mb"], workload["concurrent_files"]
            )
    ...
    def generate_plots(self, results: Dict[str, Any]):
        ...
        if "memory_pools" in results:
            ...
            if pool_data:
                ...
                pivot_pool = df_pool.pivot(
                    index="shape", columns="pool_size", values="allocation_time"
                )
        ...
        if "streaming_buffers" in results:
            ...
            if stream_data:
                ...
                axes[0, 1].plot(
                    df_stream["capacity"], df_stream["throughput"], marker="o"
                )
        ...
        if "workload_optimization" in results:
            ...
            if workload_data:
                ...
                axes[1, 0].bar(
                    df_workload["workload"], df_workload["optimization_time"]
                )
        ...
        plt.tight_layout()
        plt.savefig(
            self.output_dir / "buffer_performance.png", dpi=300, bbox_inches="tight"
        )
        plt.close()
    ...
    def save_results(self, results: Dict[str, Any]):
        ...
        if "workload_optimization" in results:
            for workload_name, metrics in results["workload_optimization"].items():
                csv_data.append(
                    {
                        "bench_type": "workload_optimization",
        ...
        if csv_data:
            ...
            csv_path = self.output_dir / "buffer_bench_results.csv"
