...
COLUMN_CONFIGS = [
    ...
    {
        "ncols": 20,
        "types": ["f4"] * 10 + ["i4"] * 5 + ["f8"] * 5,
        "name": "mixed_medium",
    },
    # Astronomical catalog configurations
    {
        "ncols": 15,
        "types": ["f8"] * 6 + ["f4"] * 6 + ["i4"] * 3,
        "name": "astrometry_catalog",
    },  # RA, DEC, proper motions, magnitudes, IDs
    {
        "ncols": 25,
        "types": ["f8"] * 10 + ["f4"] * 10 + ["i4"] * 5,
        "name": "photometry_catalog",
    },  # Multi-band photometry
    {
        "ncols": 50,
        "types": ["f4"] * 30 + ["f8"] * 10 + ["i4"] * 10,
        "name": "survey_catalog",
    },  # Full survey catalog
    {
        "ncols": 100,
        "types": ["f4"] * 50 + ["f8"] * 25 + ["i4"] * 20 + ["i1"] * 5,
        "name": "mega_catalog",
...
class TableBenchmarkSuite:
    ...
    def benchmark_torchfits(
        self, filepath: str, config: Dict, nrows: int
    ) -> Dict[str, Any]:
        ...
        if isinstance(last_data, dict):
            ...
            if columns_found != config["ncols"]:
                correctness_issues.append(
                    f"Column count mismatch: got {columns_found}, expected {config['ncols']}"
                )
                ...
                for col_name, tensor_data in last_data.items():
                    if hasattr(tensor_data, "shape"):
                        if len(tensor_data) != nrows:
                            correctness_issues.append(
                                f"Row count mismatch in {col_name}: got {len(tensor_data)}, expected {nrows}"
                            )
                        data_valid = True
                    else:
                        correctness_issues.append(
                            f"Column {col_name} is not tensor-like"
                        )
        else:
        ...
        for nrows in TABLE_SIZES:
            ...
            for config in COLUMN_CONFIGS:
                ...
                try:
                    ...
                    results["torchfits"] = self.benchmark_torchfits(
                        filepath, config, nrows
                    )
                    ...
                    if HAS_ASTROPY:
                        ...
                        results["astropy"] = self.benchmark_astropy(
                            filepath, config, nrows
                        )
                    ...
                    if HAS_FITSIO:
                        ...
                        results["fitsio"] = self.benchmark_fitsio(
                            filepath, config, nrows
                        )
    ...
    def print_performance_summary(self) -> None:
        ...
        for config_name, data in self.results.items():
            ...
            print(
                f"{config['name']:<25} {nrows:<10,} {torchfits_time:<12.4f} "
    ...
    def print_correctness_analysis(self) -> None:
        ...
        for config_name, data in self.results.items():
            ...
            if found_cols != expected_cols:
                correctness_issues.append(
                    f"{config_name}: Expected {expected_cols} columns, found {found_cols}"
                )

            # Check validity
            if not torchfits_result.get("valid", False):
                correctness_issues.append(
                    f"{config_name}: Invalid data structure returned"
                )

            # Check for errors
            if "error" in torchfits_result:
                correctness_issues.append(
                    f"{config_name}: Error - {torchfits_result['error']}"
                )
...
def main():
    ...
    parser.add_argument(
        "--quick", action="store_true", help="Run quick tests only (up to 100K rows)"
    )
