...
from torchfits.sphere.spectral import (  # noqa: E402
    alm2map,
    alm2cl,
    alm2map_spin,
    alm_index,
...
def _random_spin_alms(
    ...
    return torch.from_numpy(a_e).to(torch.complex128), torch.from_numpy(a_b).to(
        torch.complex128
    )
...
def main() -> int:
    ...
    p.add_argument(
        "--json-out", type=Path, default=DEFAULT_OUTPUT_DIR / "sphere_spectral.json"
    )
    args = p.parse_args()
    ...
    alm_vals = torch.from_numpy(rng.normal(size=nalm) + 1j * rng.normal(size=nalm)).to(
        torch.complex128
    )
    ...
    for backend in backends:
        if (
            backend == "torch"
            and args.nside >= 128
            and os.environ.get("TORCHFITS_SPIN_MAP2ALM_RING_TORCH") != "force"
        ):
            print(
                f"Skipping {backend} for nside={args.nside} (matrix path) to avoid OOM"
            )
            continue
        ...
        try:
            if False:  # Skip scalar for now to avoid OOM in matrix path
                t = _time_many(
                    lambda: map2alm(
                        map_vals, nside=args.nside, lmax=args.lmax, backend=backend_impl
                    ),
                    runs=args.runs,
                )
                rows.append(
                ...
                t = _time_many(
                    lambda: alm2map(
                        alm_vals, nside=args.nside, lmax=args.lmax, backend=backend_impl
                    ),
                    runs=args.runs,
                )
                rows.append(
                ...
                t = _time_many(
                    lambda: alm2cl(alm_vals, lmax=args.lmax, mmax=args.lmax),
                    runs=args.runs,
                )
                rows.append(
                    Row(
            ...
            r = Row(
                "map2alm_spin", backend, args.nside, args.lmax, npix, t, npix / t / 1e6
            )
            rows.append(r)
            print(
                f"{r.op:13s} {r.backend:8s} {r.nside:5d} {r.lmax:5d} {r.mops_s:8.3f} {r.time_s:8.4f}",
                flush=True,
            )
            ...
            r = Row(
                "alm2map_spin", backend, args.nside, args.lmax, npix, t, npix / t / 1e6
            )
            rows.append(r)
            print(
                f"{r.op:13s} {r.backend:8s} {r.nside:5d} {r.lmax:5d} {r.mops_s:8.3f} {r.time_s:8.4f}",
    ...
    args.json_out.write_text(
        json.dumps({"rows": [asdict(r) for r in rows]}, indent=2), encoding="utf-8"
    )
    print(f"\nJSON: {args.json_out}")
    return 0
