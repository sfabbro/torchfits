...
def bench_loader(
    ...
    *,
    ...
    print(f"{description:<28}: {pixels_sec:>12.0f} pixels/s ({mb_sec:>7.1f} MB/s)")
    return pixels_sec
...
def summarize_method(name: str, values: List[float]) -> None:
    ...
    vmax = max(values)
    print(
        f"{name:<28}: median={med:>12.0f}  mean={avg:>12.0f}  range=[{vmin:>12.0f}, {vmax:>12.0f}] pixels/s"
    )
