...
class TestTablePerformance:
    ...
    def test_tzero_floating_point_imprecision(self):
        ...
        with tempfile.NamedTemporaryFile(suffix=".fits", delete=False) as f:
            col1 = fits.Column(name='A', format='I', array=np.array([1, 2, 3], dtype=np.int16))
            col2 = fits.Column(name='B', format='J', array=np.array([1, 2, 3], dtype=np.int32))
            hdu = fits.BinTableHDU.from_columns([col1, col2])
            ...
            assert data['A'].dtype == torch.uint16, f"Expected uint16, got {data['A'].dtype}"
            assert data['B'].dtype == torch.uint32, f"Expected uint32, got {data['B'].dtype}"
