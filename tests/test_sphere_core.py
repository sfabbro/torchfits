...
def test_wrap_longitude_domains() -> None:
    lon = torch.tensor([-540.0, -190.0, -10.0, 0.0, 10.0, 359.9, 720.0], dtype=torch.float64)
...
def test_pairwise_distance_symmetry() -> None:
    ...
    torch.testing.assert_close(torch.diagonal(d), torch.zeros(4, dtype=torch.float64), atol=1e-12, rtol=0.0)
...
def test_slerp_endpoints_and_midpoint() -> None:
    ...
    torch.testing.assert_close(lon0, torch.tensor(0.0, dtype=lon0.dtype), atol=1e-12, rtol=0.0)
    torch.testing.assert_close(lat0, torch.tensor(0.0, dtype=lat0.dtype), atol=1e-12, rtol=0.0)
    torch.testing.assert_close(lon1, torch.tensor(90.0, dtype=lon1.dtype), atol=1e-12, rtol=0.0)
    torch.testing.assert_close(lat1, torch.tensor(0.0, dtype=lat1.dtype), atol=1e-12, rtol=0.0)
    torch.testing.assert_close(lonm, torch.tensor(45.0, dtype=lonm.dtype), atol=1e-10, rtol=0.0)
    torch.testing.assert_close(latm, torch.tensor(0.0, dtype=latm.dtype), atol=1e-10, rtol=0.0)
...
def test_sample_multiband_matches_healpix_interp() -> None:
    ...
    lat = torch.from_numpy(np.degrees(np.arcsin(rng.uniform(-1.0, 1.0, size=128)))).to(torch.float64)

    sampled = sample_multiband_healpix(cube, nside, lon, lat, nest=False, interpolation="bilinear")
    assert sampled.shape == (4, 128)

    expected = torch.stack(
        [hp.get_interp_val(cube[i], lon, lat, nest=False, lonlat=True) for i in range(cube.shape[0])],
        dim=0,
    )
...
def test_interpolate_wavelength_axis_linear() -> None:
    values = torch.tensor([[10.0, 20.0, 30.0], [100.0, 200.0, 300.0]], dtype=torch.float64)
...
def test_fit_remove_monopole_dipole_roundtrip() -> None:
    ...
    torch.testing.assert_close(residual, torch.zeros_like(residual), atol=1e-10, rtol=0.0)
