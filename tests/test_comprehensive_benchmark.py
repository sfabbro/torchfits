...
class BenchmarkRunner:
    ...
    def benchmark_torchfits(self, filepath: str, file_type: str, shape: Tuple[int, ...]) -> BenchmarkResult:
    ...
    def benchmark_fitsio(self, filepath: str, file_type: str, shape: Tuple[int, ...]) -> BenchmarkResult:
    ...
    def benchmark_astropy(self, filepath: str, file_type: str, shape: Tuple[int, ...]) -> BenchmarkResult:
    ...
    def benchmark_cutouts(self, filepath: str, file_type: str, shape: Tuple[int, ...]) -> List[BenchmarkResult]:
            ...
            if HAS_TORCHFITS:
                try:
                    ...
                    with MemoryMonitor() as mem:
                        ...
                        if file_type == 'image_2d':
                            slices = (slice(0, cutout_shape[0]), slice(0, cutout_shape[1]))
                        else:  # image_3d
                            slices = (slice(0, cutout_shape[0]), slice(0, cutout_shape[1]), slice(0, cutout_shape[2]))
                        ...
                        read_time_ms = (end_time - start_time) * 1000
                        memory_delta = mem.memory_delta_mb
                        cutout_size_mb = np.prod(cutout_shape) * 4 / 1024 / 1024  # Assuming float32
                        throughput_mbs = cutout_size_mb / (read_time_ms / 1000) if read_time_ms > 0 else 0
                    ...
                    with MemoryMonitor() as mem:
                        ...
                        with fitsio.FITS(filepath, 'r') as fits:
                            if file_type == 'image_2d':
                                ...
                                data = fits[0][0:cutout_shape[0], 0:cutout_shape[1], 0:cutout_shape[2]]
            ...
            if HAS_ASTROPY:
                try:
                    ...
                    with MemoryMonitor() as mem:
                        ...
                        with astropy_fits.open(filepath) as hdul:
                            if file_type == 'image_2d':
                                data = hdul[0].data[0:cutout_shape[0], 0:cutout_shape[1]]
                            else:  # image_3d  
                                data = hdul[0].data[0:cutout_shape[0], 0:cutout_shape[1], 0:cutout_shape[2]]
    ...
    def run_comprehensive_benchmark(self) -> None:
        ...
        size_categories = ['small', 'medium', 'large']  # Skip 'huge' for CI compatibility
        ...
        for file_type in file_types:
            for size_category in size_categories:
                test_count += 1
                print(f"[{test_count}/{total_tests}] Testing {file_type} ({size_category})")
                
                # Create test file
                test_file = os.path.join(self.output_dir, f"test_{file_type}_{size_category}.fits")
                try:
                    shape = FITSTestDataGenerator.create_test_file(test_file, file_type, size_category)
                ...
                for library in libraries:
                    ...
                    if result.success:
                        print(f"    ✅ {library}: {result.read_time_ms:.1f}ms, "
    ...
    def generate_report(self) -> None:
        ...
        print("-" * 80)
        print(f"{'File Type':<15} {'Size':<10} {'Library':<12} {'Time (ms)':<12} {'Throughput':<15} {'Memory':<10}")
        print("-" * 80)
        
        for file_type in ['image_2d', 'image_3d', 'spectrum_1d', 'table']:
            type_results = [r for r in success_results if r.file_type == file_type and r.operation == 'read_full']
            ...
            for size_key, size_results in size_groups.items():
                for i, result in enumerate(size_results):
                    ...
                    print(f"{file_type_display:<15} {size_display:<10} {result.library:<12} "
        ...
        if cutout_results:
            ...
            print("-" * 60)
            print(f"{'File Type':<15} {'Cutout Size':<15} {'Library':<12} {'Time (ms)':<12}")
            print("-" * 60)
            
            for result in cutout_results:
                print(f"{result.file_type:<15} {str(result.data_shape):<15} "
    ...
    def _calculate_speed_ratios(self) -> None:
        ...
        success_results = [r for r in self.results if r.success and r.operation == 'read_full']
        ...
        for (file_type, size), library_results in groups.items():
            ...
            for library, result in library_results.items():
                if library != 'fitsio':
                    ratio = fitsio_time / result.read_time_ms if result.read_time_ms > 0 else 0
...
class TestComprehensiveBenchmark:
    ...
    def test_libraries_available(self):
        ...
        assert HAS_TORCHFITS or HAS_FITSIO or HAS_ASTROPY, "No FITS libraries available for testing"
    
    def test_torchfits_available(self):
        """Test TorchFits availability"""
        if HAS_TORCHFITS:
            import torchfits
            assert hasattr(torchfits, 'read_image'), "TorchFits read_image not available"
    ...
    def test_benchmark_single_case(self, file_type, size_category, tmp_path):
        ...
        shape = FITSTestDataGenerator.create_test_file(str(test_file), file_type, size_category)
    ...
    def test_performance_comparison(self, tmp_path):
        ...
        shape = FITSTestDataGenerator.create_test_file(str(test_file), 'image_2d', 'small')
        ...
        if torchfits_result.success:
            ...
            assert torchfits_result.memory_usage_mb >= 0, "Memory usage should be non-negative"
            
            # Performance should be reasonable (less than 10 seconds for small files)
            assert torchfits_result.read_time_ms < 10000, "Read time should be reasonable"
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="TorchFits Comprehensive Benchmark Suite")
    parser.add_argument("--output-dir", help="Output directory for results", default=None)
    parser.add_argument("--quick", action="store_true", help="Run quick benchmark (small files only)")
