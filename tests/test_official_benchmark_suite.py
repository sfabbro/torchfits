...
try:
    import torchfits
    HAS_TORCHFITS = True
    print(f"✓ TorchFits available (version: {getattr(torchfits, '__version__', 'unknown')})")
...
class FITSTestDataGenerator:
    ...
    @staticmethod
    def create_image_2d(shape: Tuple[int, int], dtype=np.float32, add_noise=True) -> np.ndarray:
        ...
        if add_noise:
            ...
            for _ in range(n_stars):
                ...
                star_profile = brightness * np.exp(-((y - star_y)**2 + (x - star_x)**2) / (2 * sigma**2))
                data += star_profile
    ...
    def create_image_3d(shape: Tuple[int, int, int], dtype=np.float32) -> np.ndarray:
        ...
        for i in range(shape[0]):  # wavelength axis
            ...
            img = FITSTestDataGenerator.create_image_2d(shape[1:], dtype, add_noise=(i % 5 == 0))
    ...
    def create_spectrum_1d(length: int, dtype=np.float32) -> np.ndarray:
        ...
        for line_wave in lines:
            if line_wave > wavelength.min() and line_wave < wavelength.max():
                ...
                line_profile = line_strength * np.exp(-(wavelength - line_wave)**2 / (2 * 5**2))
    ...
    def create_table_data(n_rows: int) -> Dict[str, np.ndarray]:
        ...
        dec = np.arcsin(2 * np.random.uniform(0, 1, n_rows) - 1) * 180 / np.pi  # Proper sphere sampling
        ...
        redshift[galaxy_mask] = np.random.exponential(0.3, size=int(np.sum(galaxy_mask)))
    ...
    @classmethod
    def create_test_file(cls, filepath: str, file_type: str, size_category: str) -> Tuple[int, ...]:
        ...
        if file_type == 'image_2d':
            ...
            if HAS_ASTROPY:
                ...
                raise ImportError("Neither astropy nor fitsio available for file creation")
            ...
            if HAS_ASTROPY:
                ...
                for name, data in table_data.items():
                    if data.dtype.kind == 'U':  # String column
                        cols.append(astropy_fits.Column(name=name, format=f'{data.dtype.itemsize}A', array=data))
                    elif data.dtype == np.int64:
                        cols.append(astropy_fits.Column(name=name, format='K', array=data))
                    elif data.dtype == np.float64:
                        cols.append(astropy_fits.Column(name=name, format='D', array=data))
                    elif data.dtype == np.float32:
                        cols.append(astropy_fits.Column(name=name, format='E', array=data))
                    else:
                        # Default to double precision
                        cols.append(astropy_fits.Column(name=name, format='D', array=data.astype(np.float64)))
...
class BenchmarkRunner:
    ...
    def benchmark_torchfits(self, filepath: str, file_type: str, shape: Tuple[int, ...], 
                           operation: str = "read_full", **kwargs) -> BenchmarkResult:
        ...
        try:
            ...
            if operation == "read_full":
                if file_type in ['image_2d', 'image_3d', 'spectrum_1d']:
                    ...
                    read_func = lambda: torchfits.read(filepath, hdu=1)  # Table in extension 1
                else:
                    raise ValueError(f"Unknown file type: {file_type}")
                ...
                if file_type in ['image_2d', 'image_3d']:
                    ...
                    read_func = lambda: torchfits.read(filepath, hdu=0, start=start, shape=shape_cut)
                else:
            ...
            with MemoryMonitor() as mem:
                ...
                throughput_mbs = file_size_mb / (read_time_ms / 1000) if read_time_ms > 0 else 0
                ...
                return BenchmarkResult(
                    ...
                    success=True,
                    cutout_region=kwargs.get('cutout_region'),
                    extra_info={'device': str(getattr(data, 'device', 'cpu'))} if hasattr(data, 'device') else None
                )
    ...
    def benchmark_fitsio(self, filepath: str, file_type: str, shape: Tuple[int, ...], 
                        operation: str = "read_full", **kwargs) -> BenchmarkResult:
                ...
                def read_func():
                    with fitsio.FITS(filepath, 'r') as fits:
                        if file_type == 'image_2d':
                            ...
                            return fits[0][y_slice.start:y_slice.stop, x_slice.start:x_slice.stop]
                        elif file_type == 'image_3d':
                            z_slice, y_slice, x_slice = cutout_region
                            return fits[0][z_slice.start:z_slice.stop, 
    ...
    def benchmark_astropy(self, filepath: str, file_type: str, shape: Tuple[int, ...], 
                         operation: str = "read_full", **kwargs) -> BenchmarkResult:
    ...
    def run_comparison_benchmark(self, filepath: str, file_type: str, shape: Tuple[int, ...], 
                               operation: str = "read_full", **kwargs) -> List[BenchmarkResult]:
        """Run benchmark comparison across all libraries"""
        results = []
        
        # Benchmark TorchFits
        result_tf = self.benchmark_torchfits(filepath, file_type, shape, operation, **kwargs)
        results.append(result_tf)
        
        # Benchmark fitsio
        result_fitsio = self.benchmark_fitsio(filepath, file_type, shape, operation, **kwargs)
        results.append(result_fitsio)
        
        # Benchmark astropy
        result_astropy = self.benchmark_astropy(filepath, file_type, shape, operation, **kwargs)
        ...
        if result_tf.success:
            if result_fitsio.success and result_fitsio.read_time_ms > 0:
                result_tf.speedup_vs_baseline = result_fitsio.read_time_ms / result_tf.read_time_ms
            elif result_astropy.success and result_astropy.read_time_ms > 0:
                result_tf.speedup_vs_baseline = result_astropy.read_time_ms / result_tf.read_time_ms
...
def test_files(benchmark_runner):
    ...
    for file_type, size_category in test_configs:
        ...
        try:
            shape = FITSTestDataGenerator.create_test_file(filepath, file_type, size_category)
            ...
            print(f"✓ Created {filename}: shape={shape}, size={os.path.getsize(filepath)/1024/1024:.1f}MB")
        except Exception as e:
...
def test_image_performance(benchmark_runner, test_files, file_key):
    ...
    if HAS_TORCHFITS:
        assert torchfits_result.success, f"TorchFits failed: {torchfits_result.error_message}"
        ...
        if torchfits_result.speedup_vs_baseline:
            ...
            if file_info['size'] in ['medium', 'large']:
                ...
                if torchfits_result.speedup_vs_baseline < 0.8:
                    print(f"Warning: Performance below 0.8x baseline: {torchfits_result.speedup_vs_baseline:.2f}x")
                # Still assert reasonable performance
                assert torchfits_result.speedup_vs_baseline > 0.5, f"Performance too low: {torchfits_result.speedup_vs_baseline:.2f}x"
...
def test_table_performance(benchmark_runner, test_files, file_key):
    ...
    if HAS_TORCHFITS:
        ...
        assert len(torchfits_result.data_shape) == 2, "Table should have 2D shape (rows, columns)"
        assert torchfits_result.data_shape[1] > 5, "Should have multiple columns"


@pytest.mark.parametrize("file_key,cutout_spec", [
...
def test_cutout_performance(benchmark_runner, test_files, file_key, cutout_spec):
    ...
    results = benchmark_runner.run_comparison_benchmark(
        file_info['path'], file_info['type'], file_info['shape'], 
        operation="read_cutout", cutout_region=cutout_spec
    )
    ...
    if HAS_TORCHFITS and torchfits_result and torchfits_result.success:
        ...
        assert torchfits_result.read_time_ms < 100, f"Cutout took {torchfits_result.read_time_ms:.1f}ms, expected <100ms"
        
        # Verify cutout shape
        expected_shape = tuple(s.stop - s.start for s in cutout_spec)
        if file_info['type'] == 'image_2d':
            assert torchfits_result.data_shape == expected_shape, f"Expected shape {expected_shape}, got {torchfits_result.data_shape}"
        elif file_info['type'] == 'image_3d':
            assert torchfits_result.data_shape == expected_shape, f"Expected shape {expected_shape}, got {torchfits_result.data_shape}"
        
        print(f"Cutout performance: {torchfits_result.read_time_ms:.2f}ms for {expected_shape}")


@pytest.mark.parametrize("file_key,columns", [
    ("table_small", ['RA', 'DEC']),
...
def test_column_selection_performance(benchmark_runner, test_files, file_key, columns):
    ...
    results = benchmark_runner.run_comparison_benchmark(
        file_info['path'], file_info['type'], file_info['shape'],
        operation="read_columns", columns=columns
    )
    ...
    if HAS_TORCHFITS and torchfits_result and torchfits_result.success:
        ...
        assert torchfits_result.read_time_ms < 50, f"Column selection took {torchfits_result.read_time_ms:.1f}ms, expected <50ms"
        
        # Verify correct number of columns
        assert torchfits_result.data_shape[1] == len(columns), f"Expected {len(columns)} columns, got {torchfits_result.data_shape[1]}"
        
        print(f"Column selection performance: {torchfits_result.read_time_ms:.2f}ms for {len(columns)} columns")
...
def test_memory_efficiency(benchmark_runner, test_files):
    ...
    torchfits_result = next((r for r in results if r.library == "torchfits" and r.success), None)
    ...
    if torchfits_result and other_results:
        ...
        if min_other_memory > 0:
            ...
            print(f"Memory efficiency: TorchFits uses {memory_ratio:.2f}x memory vs best competitor")
            
            # Memory usage should be competitive
            assert memory_ratio < 2.0, f"Memory usage too high: {memory_ratio:.2f}x vs competitors"
...
def test_error_handling(benchmark_runner, test_files):
    ...
    if "image_2d_small" in test_files:
        ...
        result = benchmark_runner.benchmark_torchfits(
            file_info['path'], file_info['type'], file_info['shape'],
            operation="read_cutout", cutout_region=invalid_cutout
        )
...
def test_performance_summary(benchmark_runner):
    ...
    torchfits_results = [r for r in benchmark_runner.results if r.library == "torchfits" and r.success]
    fitsio_results = [r for r in benchmark_runner.results if r.library == "fitsio" and r.success]
    astropy_results = [r for r in benchmark_runner.results if r.library == "astropy" and r.success]
    ...
    if torchfits_results:
        ...
        for result in torchfits_results:
            speedup_str = f" ({result.speedup_vs_baseline:.2f}x speedup)" if result.speedup_vs_baseline else ""
            print(f"  {result.file_type}_{result.operation}: {result.read_time_ms:.2f}ms{speedup_str}")
    
    # Calculate overall statistics
    if torchfits_results:
        speedups = [r.speedup_vs_baseline for r in torchfits_results if r.speedup_vs_baseline]
        if speedups:
            ...
            assert avg_speedup > 1.0, f"Overall performance should be better than baseline, got {avg_speedup:.2f}x"
    
    # Save detailed results
    results_file = os.path.join(benchmark_runner.output_dir, "benchmark_results.json")
    with open(results_file, 'w') as f:
        json.dump([asdict(r) for r in benchmark_runner.results], f, indent=2, default=str)
