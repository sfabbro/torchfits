...
def mock_cpp():
    with mock.patch("torchfits.io.cpp") as m_cpp:
        ...
        m_cpp.read_hdus_batch.return_value = [
            torch.zeros((10, 10)),
            torch.zeros((10, 10)),
        ]
        m_cpp.read_images_batch.return_value = [
            torch.zeros((10, 10)),
            torch.zeros((10, 10)),
        ]
        # read_full_raw_with_scale returns: data, scaled, bscale, bzero
        m_cpp.read_full_raw_with_scale.return_value = (
            torch.zeros((10, 10)),
            False,
            1.0,
            0.0,
...
class TestIORead:
    ...
    def test_read_invalid_path_type(self):
        ...
        with pytest.raises(
            ValueError, match="Path must be a string or list of strings"
        ):
            io.read(123)
    ...
    def test_read_batch_path_invalid_hdu(self):
        ...
        with pytest.raises(
            ValueError, match="Batch read requires a single integer HDU"
        ):
    ...
    def test_read_invalid_hdu_type(self):
        ...
        with pytest.raises(
            ValueError, match="HDU index must be a non-negative integer"
        ):
