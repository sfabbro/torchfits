...
def to_pandas(data: Dict[str, Any]) -> "pd.DataFrame":
    ...
    for key, value in data.items():
        if isinstance(value, torch.Tensor):
            ...
            processed_data[key] = [t.numpy() if isinstance(t, torch.Tensor) else t for t in value]
        else:
...
def to_arrow(data: Dict[str, Any]) -> "pa.Table":
    ...
    for key, value in data.items():
        ...
        if isinstance(value, torch.Tensor):
            ...
            converted_list = [t.numpy() if isinstance(t, torch.Tensor) else t for t in value]
