...
class WCS:
    ...
    def __init__(self, header: Header):
        ...
        if not hasattr(cpp, "WCS"):
            raise ImportError("WCS support not available. torchfits was built without wcslib.")
    ...
    def to(self, device: torch.device) -> "WCS":
        ...
        if self._sip_a is not None: self._sip_a = self._sip_a.to(device)
        if self._sip_b is not None: self._sip_b = self._sip_b.to(device)
        if self._sip_ap is not None: self._sip_ap = self._sip_ap.to(device)
        if self._sip_bp is not None: self._sip_bp = self._sip_bp.to(device)
        return self
    ...
    def center_coord(self) -> Tensor:
        ...
        center_pix = torch.tensor([[naxis1 / 2.0, naxis2 / 2.0]], dtype=torch.float64, device=self._crpix.device)
        return self.pixel_to_world(center_pix).squeeze()

    @property
    def crpix(self) -> Tensor: return self._crpix
    @property
    def crval(self) -> Tensor: return self._crval
    @property
    def cdelt(self) -> Tensor: return self._cdelt
    @property
    def pc(self) -> Tensor: return self._pc
    @property
    ...
    def _apply_sip(self, xy: Tensor, coeffs_x: Optional[Tensor], coeffs_y: Optional[Tensor]) -> Tensor:
        ...
        # u_pow[p] = u^p
        max_order = 0
        if coeffs_x is not None: max_order = max(max_order, coeffs_x.shape[0]-1)
        if coeffs_y is not None: max_order = max(max_order, coeffs_y.shape[0]-1)
    ...
    def pixel_to_world(self, pixels: Tensor, batch_size: Optional[int] = None) -> Tensor:
        ...
        if batch_size is not None and pixels.shape[0] > batch_size:
            ...
            for i in range(0, pixels.shape[0], batch_size):
                ...
                results.append(self.pixel_to_world(batch))
    ...
    def world_to_pixel(self, coords: Tensor, batch_size: Optional[int] = None) -> Tensor:
        ...
        if batch_size is not None and coords.shape[0] > batch_size:
            ...
            for i in range(0, coords.shape[0], batch_size):
                ...
                results.append(self.world_to_pixel(batch))
            return torch.cat(results, dim=0)

        # Fallback to wcslib (CPU) for non-standard cases
        is_tan = (self.naxis == 2 and
                  self._ctype[0].endswith("TAN") and
                  self._ctype[1].endswith("TAN"))
