...
def project_allsky(xi: Tensor, eta: Tensor, projection_code: str, params: Optional[Dict[str, float]] = None) -> Tuple[Tensor, Tensor]:
    """
    ...
    if projection_code == 'AIT':
        ...
        if mask_pol.any():
            ...
            dx = xi_p - xc
            sigma_safe = torch.where(torch.abs(sigma) < 1e-9, torch.ones_like(sigma), sigma)
        ...
        theta = torch.where(invalid_eta, nan_like, theta)
         ...
         raise NotImplementedError(f"All-Sky projection {projection_code} not implemented")
