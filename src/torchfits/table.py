...
class ColumnInfo:
    ...
    @classmethod
    def from_fits_header(cls, colname: str, header_dict: Dict[str, Any], dtype: torch.dtype) -> 'ColumnInfo':
        """
        ...
        fits_metadata = {k: v for k, v in header_dict.items() 
...
class FitsTable:
            ...
            for col_name, col_meta in metadata.items():
                if isinstance(col_meta, ColumnInfo):
                    ...
                    self.column_info[col_name] = ColumnInfo(
                        ...
                        **{k: v for k, v in col_meta.items() if k not in ['unit', 'description']}
                    )
    ...
    def __getitem__(self, key) -> Union[torch.Tensor, 'FitsTable']:
        ...
        if isinstance(key, str):
            ...
            if key.shape[0] != self._length:
                raise ValueError(f"Boolean mask length {key.shape[0]} doesn't match table length {self._length}")
    ...
    def select(self, columns: Union[str, List[str]]) -> 'FitsTable':
        ...
        selected_metadata = {col: self.column_info.get(col) for col in columns if col in self.column_info}
    ...
    def filter(self, mask: torch.Tensor) -> 'FitsTable':
        ...
        if mask.shape[0] != self._length:
            raise ValueError(f"Mask length {mask.shape[0]} doesn't match table length {self._length}")
    ...
    def convert_units(self, column: str, target_unit: str) -> 'FitsTable':
        ...
        if factor is None:
            raise ValueError(f"Conversion from '{current_unit}' to '{target_unit}' not supported")
        ...
        for col_name, col_data in other.data.items():
            if col_name != on:  # Don't duplicate join column
                ...
                if how == 'left' and -1 in [idx for idx in right_indices if idx >= 0]:
                    ...
                    result_data[result_col_name] = col_data[right_indices[right_indices >= 0]]
                else:
...
class GroupedFitsTable:
    ...
    def mean(self) -> FitsTable:
        ...
        for name, tensor in self.table.data.items():
            if name != self.group_column and tensor.dtype in [torch.float32, torch.float64, torch.int32, torch.int64]:
    ...
    def count(self) -> FitsTable:
        ...
        first_col = next(name for name in self.table.columns if name != self.group_column)
