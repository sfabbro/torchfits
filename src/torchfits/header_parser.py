    ...
    def parse_header_string(cls, header_string: str) -> Dict[str, Any]:
        ...
        for i in range(0, len(header_string), 80):
            ...
            if len(card) > 8 and card[8] == "=":
                value_comment = card[9:]

                # Find comment separator
                idx = value_comment.find("/")
                if idx != -1:
                    # Fast check if there are no quotes before the slash
                    if value_comment.find("'", 0, idx) == -1:
                        comment_start = idx
                    else:
                        # Full quoted string check
                        while idx != -1:
                ...
                # Parse the value and set to header
                header[keyword] = cls._parse_value(value_str, keyword)
            else:
