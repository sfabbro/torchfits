...
def to_tensor_frame(data: Dict[str, torch.Tensor]) -> TensorFrame:
    ...
    if not HAS_TORCH_FRAME:
        raise ImportError("torch-frame is not installed. Please install it with 'pip install pytorch-frame'.")
    ...
    for name, tensor in data.items():
        if num_rows == -1:
            ...
            raise ValueError(f"Column {name} has {tensor.shape[0]} rows, expected {num_rows}")
            
        # Determine stype based on dtype
        if tensor.dtype in [torch.float32, torch.float64, torch.int32, torch.int16, torch.uint8]:
            ...
            if tensor.dim() == 1:
                ...
                warnings.warn(f"Skipping column {name} with dimension {tensor.dim()} (only 1D/2D supported)")
                continue
                
            num_cols.append(tensor.to(torch.float32)) # torch-frame expects float for numerical
            ...
            warnings.warn(f"Skipping column {name} with unsupported dtype {tensor.dtype}")
...
def read_tensor_frame(path: str, hdu: int = 1, columns: Optional[List[str]] = None) -> TensorFrame:
    """
